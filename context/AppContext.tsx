...
const DEFAULT_USER: User = {
  ...
  email: 'developer@abiltube.app',
  avatar: '/friends/saddam.jpg',
  handle: '@abiltube_creator',
};
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Sync dark mode class and color-scheme with HTML element
  useEffect(() => {
    if (isDarkMode) {
  ...
  const playVideoById = (id: string, fallbackVideo?: Video) => {
    ...
    if (!found && id.startsWith('yt-')) {
      ...
      found = {
        ...
        youtubeId: ytId,
        title: 'YouTube Video',
        description: 'Watch video seamlessly on AbilTube.',
  ...
  const notifications: NotificationItem[] = useMemo(() => {
    ...
    const uniqueContent: Video[] = [];

    for (const item of [...videos, ...shorts, ...Object.values(savedVideosMap)]) {
      if (!item || !item.id) continue;
      const dedupeKey = item.youtubeId || item.id;
      if (!seenIds.has(dedupeKey) && !seenIds.has(item.id)) {
    ...
    const subscribedContent = uniqueContent.filter((item) => {
